                ...
                case SDK_PAY_FLAG: {
                    ...
                    if (TextUtils.equals(resultStatus, "9000")) {
                        ...
                        if (TextUtils.equals(resultStatus, "8000")) {
                            Toast.makeText(LikeDetailActivity.this, "支付结果确认中", Toast.LENGTH_SHORT).show();
                        } else {
                            // 其他值就可以判断为支付失败，包括用户主动取消支付，或者系统返回的错误
                            Toast.makeText(LikeDetailActivity.this, "支付宝支付取消", Toast.LENGTH_SHORT).show();
                        }
                    }
                    break;
                }
    ...
    private void showListDialog(final int adapterPosition, boolean isfav, boolean islive, final int id) {
        ...
        mDialog.getView(R.id.tv_download).setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                int apnType = NetWorkUtils.getAPNType(LikeDetailActivity.this);
                if (apnType == 0) {
                    Toast.makeText(LikeDetailActivity.this, "没有网络呢~", Toast.LENGTH_SHORT).show();
                    return;
                } else if (apnType == 2 || apnType == 3 || apnType == 4) {
                    nowifiallowdown = SpUtils.getBoolean(LikeDetailActivity.this, "nowifiallowdown", false);
                    if (!nowifiallowdown) {
                        ...
                        mNetDialog.getView(R.id.tv_yes).setOnClickListener(new View.OnClickListener() {
                            ...
                            public void onClick(View v) {//记住用户允许流量下载
                                SpUtils.putBoolean(LikeDetailActivity.this, "nowifiallowdown", true);
    ...
    private void goPay(String order_sn, final String type) {
        ...
        if (type.equals("1")) {//余额支付
            ...
            OkGo.<AliPayBean>post(MyContants.LXKURL + "order/pay")
                    ...
                    .headers(headers)
                    .params(params)
                    .execute(new DialogCallback<AliPayBean>(LikeDetailActivity.this, AliPayBean.class) {
                                 @Override
                                 public void onSuccess(Response<AliPayBean> response) {
                                     ...
                                     Runnable payRunnable = new Runnable() {
                                         ...
                                         @Override
                                         public void run() {
                                             PayTask alipay = new PayTask(LikeDetailActivity.this);
